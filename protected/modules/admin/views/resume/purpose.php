...
<div class="tab">
    ...
    <a  class="on" href="javascript:;">求职意向</a>
    <a href="<?php echo $this->createUrl('/admin/resume/education', array('id' => $id)); ?>">教育背景</a>
...
<div class="resume_list">
    <?php foreach ($allModel as $v): ?>
        <table class="items table table-bordered">
            ...
            <tbody>
                <tr>
                    <td><?php echo UserPurposeModel::model()->apply_job_type_arr($v->apply_job_type_id); ?></td><td><a href="<?php echo $this->createUrl('purpose', array('id' => $id, 'purpose_id' => $v->id)) ?>">修改</a></td>
                </tr>
            </tbody>
        </table>
    <?php endforeach ?>
        ...
        <table>
            <tbody>
                <tr>
                    <td>求职类型：</td>
                    <td>
                        <?php echo $form->dropDownList($model, 'apply_job_type_id', UserPurposeModel::model()->apply_job_type_arr(), array('prompt' => '请选择', 'id' => 'apply_job_type')); ?>
                    </td>
                </tr>
            </tbody>
            <tbody class="full-time">
                <tr <?php if (array_key_exists('wish_job_place_id', $resume_log_content)): ?> class="<?php echo $resume_log_content['wish_job_place_id']['color'] ?>" <?php endif ?>>
                    <td>期望工作地点：</td>
                    <td>
                        <?php echo $form->dropDownList($model, 'wish_job_place_id', UserPurposeModel::model()->get_area(), array('prompt' => '请选择')); ?>
                    </td>
                </tr>
                <tr <?php if (array_key_exists('wish_treatment_id', $resume_log_content)): ?> class="<?php echo $resume_log_content['wish_treatment_id']['color'] ?>" <?php endif ?>>
                    <td>期待待遇：</td>
                    <td>
                        <?php echo $form->dropDownList($model, 'wish_treatment_id', UserPurposeModel::model()->wish_treatment_arr(), array('prompt' => '请选择')); ?>
                    </td>
                </tr>
                <tr <?php if (array_key_exists('other_weal', $resume_log_content)): ?> class="<?php echo $resume_log_content['other_weal']['color'] ?>" <?php endif ?>>
                    <td>其他福利：</td>
                    <td class="other_weal">
                        <?php echo $form->checkBoxList($model, 'other_weal', UserPurposeModel::model()->get_other_weal(), array('separator' => '&nbsp;&nbsp;&nbsp;&nbsp;')) ?>
                    </td>
                </tr>
                <tr <?php if (array_key_exists('resume_position_id', $resume_log_content)): ?> class="<?php echo $resume_log_content['resume_position_id']['color'] ?>" <?php endif ?>>
                    <td>应聘职位：</td>
                    <td>
                        <?php echo $form->dropDownList($model, 'resume_position_id', UserPurposeModel::model()->get_position(), array('prompt' => '请选择')); ?>
                    </td>
                </tr>
            </tbody>
            <tbody class="part-time">
                <tr <?php if (array_key_exists('wish_part_time_treatment_id', $resume_log_content)): ?> class="<?php echo $resume_log_content['wish_part_time_treatment_id']['color'] ?>" <?php endif ?>>
                    <td>期待待遇：</td>
                    <td>
                        <?php echo $form->dropDownList($model, 'wish_part_time_treatment_id', UserPurposeModel::model()->wish_part_time_treatment_arr(), array('prompt' => '请选择')); ?>
                    </td>
                </tr>
                <tr <?php if (array_key_exists('part_time_type', $resume_log_content)): ?> class="<?php echo $resume_log_content['part_time_type']['color'] ?>" <?php endif ?>>
                    <td>期望类型：</td>
                    <td class="other_weal">
                        <?php echo $form->checkBoxList($model, 'part_time_type', UserPurposeModel::model()->get_part_time_type(), array('separator' => '&nbsp;')) ?>
                    </td>
                </tr>
            </tbody>
                <?php else: ?>
...
<?php if (isset($model->apply_job_type_id) && $model->apply_job_type_id == 1): ?>
            $('.part-time').show();
            $('.full-time').hide();
<?php elseif (isset($model->apply_job_type_id) && $model->apply_job_type_id > 1): ?>
            $('.part-time').hide();
            $('.full-time').show();
<?php endif; ?>
    })
